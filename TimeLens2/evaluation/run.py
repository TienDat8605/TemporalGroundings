...
def main():
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            try:
                ...
                judge_kwargs = {
                    ...
                    'verbose': args.verbose,
                    'retry': args.retry if args.retry is not None else 99999999,
                ...
                failed_combinations.append((model_name, dataset_name, f'{type(e).__name__}: {e}'))
                continue
    ...
    if failed_combinations:
        summary = '; '.join(
            f'{model_name} x {dataset_name}: {error}'
            for model_name, dataset_name, error in failed_combinations
        )
        raise RuntimeError(f'{len(failed_combinations)} model/dataset combination(s) failed: {summary}')
