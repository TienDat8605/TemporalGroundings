    ...
    def calc_frame_info(self, data_item):
        ...
        if len(self._video_extra_info_list) > 0:
            for video_extra_info in self._video_extra_info_list:
                ...
                # 在提供processed_video_length和processed_fps时，origin_fps和origin_video_length可以不提供
                origin_fps = video_extra_info.get("origin_fps", None)
                origin_video_length = video_extra_info.get("origin_video_length", None)
                if origin_fps is not None:
                    origin_fps_list.append(origin_fps)

                processed_video_length = video_extra_info.get("processed_video_length")
                processed_fps = video_extra_info.get("processed_fps")

                if origin_fps is None and origin_video_length is None:
                    assert processed_video_length is not None and processed_fps is not None, (
                        f"processed_video_length and processed_fps must both exist when origin_fps and origin_video_length are not provided, "
                        f"data_name: {self.data_name}, data_id: {data_item.get('id', '')}. Discard this data."
                    )
                # 两个要不都存在，要么都不存在
