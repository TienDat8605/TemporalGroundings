...
The optional downloader retrieves, but does not redistribute, upstream assets. Qwen, CLIP, PyTorchVideo pretrained weights, UniVTG checkpoints, TimeLens2 checkpoints, OMTG, TACoS, and QVHighlights retain their independent licenses and dataset/source-video terms. TACoS is retrieved from VideoMind's BSD-3-Clause dataset repository using its 3 FPS, 480p, no-audio variant; the underlying TACoS video data remains limited to scientific use by MPII and may not be republished. QVHighlights test annotations come from Moment-DETR, and only their referenced videos are selected from the CC BY-NC-SA 4.0 `ayushsdev/qvhighlights-videos` repository.
