...
SESSION_NAME="${OMTG_TMUX_SESSION:-omtg-vtg-10}"
ASSET_ROOT="${OMTG_ASSET_ROOT:-$PROJECT_ROOT/assets}"
GPU="${OMTG_GPU:-0}"
SEED="${OMTG_SEED:-42}"
SUBSET="${OMTG_SUBSET:-10}"
TOP_K="${OMTG_CORRIDOR_TOP_K:-4}"
RERUN="${OMTG_RERUN:-0}"
LOG_ROOT="${OMTG_LOG_ROOT:-$PROJECT_ROOT/results/logs/omtg-adaptive-pruned}"
[[ "$ASSET_ROOT" == /* ]] || ASSET_ROOT="$PROJECT_ROOT/$ASSET_ROOT"
[[ "$LOG_ROOT" == /* ]] || LOG_ROOT="$PROJECT_ROOT/$LOG_ROOT"

usage() {
  cat <<'EOF'
Run UniTime and TimeLens-8B adaptive + Mage + SemVID on OMTG in detached tmux.

Usage:
  scripts/run_omtg_tmux.sh

Optional environment variables:
  OMTG_TMUX_SESSION     tmux session name (default: omtg-vtg-10)
...
run_worker() {
  ...
  echo "subset: $SUBSET; seed: $SEED; corridor top-k: $TOP_K"
  ...
  local timelens8="$ASSET_ROOT/checkpoints/timelens-8b"
  local -a missing=()
  for path in "$data" "$unitime_adapter" "$unitime_base" "$timelens8"; do
    [[ -e "$path" ]] || missing+=("$path")
  done
  if ((${#missing[@]})); then
    printf 'missing required asset: %s\n' "${missing[@]}" >&2
    echo "download first with:" >&2
    echo "hybrid-vtg download omtg unitime timelens-8b --root '$ASSET_ROOT' --accept-licenses --hf-login" >&2
    exit 1
  fi
  ...
  run_one unitime-adaptive-mage-semvid \
    --model unitime --checkpoint "$unitime_adapter" --base-checkpoint "$unitime_base"
  run_one timelens8-adaptive-mage-semvid \
    --model timelens-8b --checkpoint "$timelens8"
